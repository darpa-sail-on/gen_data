...
def get_known_and_unknown(train, test, novelty_type):
    if novelty_type == 'relation' or novelty_type == 'relation_type':
        known_classes = set(train[novelty_type + '1']).union(
            set(train[novelty_type + '2']))
        unknown_classes = set(test[novelty_type + '1']).union(
            (test[novelty_type + '2']))
        unknown_classes -= known_classes
    else:
        assert novelty_type in ['class','perspective', 'location']
        known_classes = set(train[novelty_type])
        unknown_classes = set(test[novelty_type]) - known_classes
...
def create_individual_test(known_videos, unknown_videos,
    ...
    if aug_type == "spatial":
        ...
        is_spatial = 0
        is_temporal = 1
    elif aug_type in ['class','perspective',
...
@click.option("--aug_type", default="class",
    type=click.Choice(['class','perspective', 'location',
